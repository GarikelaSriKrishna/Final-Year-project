...
# Function to process Facebook dataset and train a single hybrid model
def process_facebook(dataset_path):
    print("Processing Facebook dataset...")
    
    # Load dataset
    data = pd.read_csv(dataset_path)
    X = data.drop('Label', axis=1)  # Assuming 'is_fake' is the target column
    y = data['Label']
    ...
    print(f"Facebook Model Accuracy: {accuracy:.4f}")
    print(f"Facebook Model AUC-ROC: {auc_score:.4f}")
    print("Classification Report:")
    print(report)
    
    # Save model and results
    results_folder = 'results/facebook'
    models_folder = 'models/facebook'
    ...
    with open(os.path.join(results_folder, 'classification_report.txt'), 'w') as f:
        f.write(f"Facebook Model Accuracy: {accuracy:.4f}\n")
        f.write(f"Facebook Model AUC-ROC: {auc_score:.4f}\n")
...
# Facebook dataset path
facebook_path = 'datasets/facebook_dataset.csv'  # Update with actual path

# Process Facebook dataset
process_facebook(facebook_path)